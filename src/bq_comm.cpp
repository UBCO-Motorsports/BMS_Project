...
BMSErrorCode_t bqAutoAddressStack() {
    ...
    for (uint16_t reg = OTP_ECC_DATAIN1; reg <= OTP_ECC_DATAIN8; ++reg) {
        ...
        if (status != BMS_OK) {
            BMS_DEBUG_PRINTF("DLL Sync Write failed for reg 0x%X\n", reg);
            return status;
        }
    }
    ...
    if (status != BMS_OK) {
        Serial.println("Enable auto-address mode failed");
        return status;
    }
    ...
    for (uint8_t i = 0; i < TOTAL_BQ_DEVICES; ++i) { // Bridge (0) + segments (1 to N)
        ...
        if (status != BMS_OK) {
            BMS_DEBUG_PRINTF("Setting address %d failed\n", i);
            return status;
        }
    }
    ...
    if (status != BMS_OK) {
        Serial.println("Configure stack devices failed");
        return status;
    }
    ...
    if (NUM_BQ79616_DEVICES > 0) {
        ...
        if (status != BMS_OK) {
            BMS_DEBUG_PRINTF("Configure ToS device (Addr %d) failed\n", tos_address);
            return status;
        }
    }
    ...
    for (uint16_t reg = OTP_ECC_DATAIN1; reg <= OTP_ECC_DATAIN8; ++reg) {
        ...
        if (status != BMS_OK &&
            status != BMS_ERROR_CRC &&
            status != BMS_ERROR_COMM_TIMEOUT) {
            BMS_DEBUG_PRINTF(
                "Warning: Dummy stack read for DLL sync failed for reg 0x%X with status %d\n",
                reg, status
            );
    ...
    for (uint8_t i = 0; i < NUM_BQ79616_DEVICES; ++i) {
        Serial.print("    Dev ");
        Serial.print(i + 1);
        Serial.print(" reports 0x");
        Serial.println(addrBuf[i], HEX);
    }
    ...
    Serial.print("    DEV_CONF1 = 0x");
    Serial.println(devConf1, HEX);
    if (devConf1 != 0x14) {
    ...
    if (status != BMS_OK) {
        Serial.println("Resetting FAULT_RST2 failed");
        return status;
    }
    ...
    if (status != BMS_OK) {
        Serial.println("Resetting FAULT_RST1 failed");
        return status;
    } 
